...
原作者[仓库链接](https://github.com/yangjunlong/barrager.js)

原仓库代码的弹幕样式是和直播一样横着飘过去，以及有输入框修改直播间ID和发送弹幕的功能；

我自己删掉了上述不需要的功能，改成只有一个空白网页每条弹幕竖着列出来，并加上了弹幕发送者的昵称和粉丝牌、进入直播间提醒、关注直播间提醒、礼物提醒等其他功能。

使用方法：直接在浏览器中访问 `index.html` 并带上 `?roomId=` 参数即可。
